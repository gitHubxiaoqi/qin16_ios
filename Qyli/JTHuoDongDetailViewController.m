//
//  JTHuoDongDetailViewController.m
//  Qyli
//
...
@interface JTHuoDongDetailViewController ()<UIWebViewDelegate>
{
    int p;
...
@implementation JTHuoDongDetailViewController
...
-(void)readyUIAgain
{
    ...
    [kaquanBtn addTarget:self action:@selector(baoMing) forControlEvents:UIControlEventTouchUpInside];
    ...
    [jigouBtn addTarget:self action:@selector(jigou) forControlEvents:UIControlEventTouchUpInside];
...
-(void)baoMing
{
    ...
    JTHuoDongBaoMingViewController * bmVC=[[JTHuoDongBaoMingViewController alloc] init];
    bmVC.title=_model.title;
    bmVC.infoID=_model.idStr;
...
-(void)sendPost
{
    ...
    if ([SOAPRequest checkNet])
    {
        ...
        NSDictionary * zaojiaoDic=[[NSDictionary alloc] initWithDictionary:[SOAPRequest getResponseByPostURL:[NSString stringWithFormat:@"%s%s",QYL_URL,NEW_QYL_HuoDong_activityInfo] jsonDic:jsondic]];
        
        if ([[zaojiaoDic objectForKey:@"resultCode"] intValue]==1000)
        {
            NSDictionary * resultDic=[[NSDictionary alloc] initWithDictionary:[zaojiaoDic objectForKey:@"iaDTO"]];
            _model=[[JTSortModel alloc] init];
            _model=self.sortmodel;
            _model.imgUrlStr=[resultDic objectForKey:@"pickey"];
...
//            _model.beginDate=[resultDic objectForKey:@"beginAge"];
//            _model.endDate=[resultDic objectForKey:@"endAge"];
//            _model.cost=[NSString stringWithFormat:@"%@元",[resultDic objectForKey:@"fee"]];
            _model.description1=[resultDic objectForKey:@"description"];
            _model.openTime=[resultDic objectForKey:@"time"];
