...
@interface ChatViewController ()<UIAlertViewDelegate,UITableViewDelegate,UITableViewDataSource, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate,IDeviceManagerDelegate>
...
//    [self.view addSubview:bigBgImgview];
    ...
    UIImageView * deleteImgView=[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"msg_delete_post.png"]];
    deleteImgView.frame=CGRectMake(20, 12,20, 20);
    [deleteBtn addSubview:deleteImgView];
...
//    } onQueue:nil];
//    
//    [[NSUserDefaults standardUserDefaults] setObject:@"2" forKey:@"other"];
        ...
        if (imageBody.thumbnailDownloadStatus == EMAttachmentDownloadSuccessed) {
            if (imageBody.attachmentDownloadStatus == EMAttachmentDownloadSuccessed)
            {
                ...
                if (localPath && localPath.length > 0) {
                    ...
                    self.messageReadManager.chatVC=self;
     ...
     asyncStopRecordingAudioWithCompletion:^(EMChatVoice *aChatVoice, NSError *error){
         if (!error) {
             ...
             if (error.code == EMErrorAudioRecordNotStarted) {
                 [self showHint:@"录音没有开始" yOffset:-40];
             } else {
...
-(void)sendTextMessage:(NSString *)textMessage
{
    ...
    JTAppDelegate * appdelegate=[UIApplication sharedApplication].delegate;
 if(self.shangjiaName==nil||self.shangjiaPic==nil)
 {
     if ([_conversation latestMessageFromOthers]==nil)
     {
         NSDictionary * dic=[[NSDictionary alloc] initWithDictionary: [_conversation latestMessage].ext];
         self.shangjiaPic=[dic objectForKey:@"reciverPic"];
         self.shangjiaName=[dic objectForKey:@"reciverName"];
     }
     else
     {
         NSDictionary * dic=[[NSDictionary alloc] initWithDictionary: [_conversation latestMessageFromOthers].ext];
         self.shangjiaPic=[dic objectForKey:@"senderPic"];
         self.shangjiaName=[dic objectForKey:@"senderName"];
     }

 }
    NSDictionary * dic=[[NSDictionary alloc] initWithObjectsAndKeys:appdelegate.appUser.headPortraitImgUrlStr,@"senderPic",appdelegate.appUser.userName,@"senderName", self.shangjiaPic,@"reciverPic",self.shangjiaName,@"reciverName",nil];
