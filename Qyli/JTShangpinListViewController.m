//
//  JTShangpinListViewController.m
//  Qyli
//
...
@interface JTShangpinListViewController ()<UITableViewDataSource,UITableViewDelegate>
@end
@implementation JTShangpinListViewController
{
    int p;
    int q;
    int m;
...
-(void)leftBtnCilck
{
    ...
    //    JTTabBarViewController * mainVC=[[JTTabBarViewController alloc] init];
    //    UIWindow * window=[UIApplication sharedApplication].keyWindow;
...
-(float)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (tableView.tag-100)
    {
        case 0:
        {
            ...
            CGSize size=CGSizeMake(self.view.frame.size.width-10-150-70, MAXFLOAT);
            CGSize autoSize=[[NSString stringWithFormat:@"%@",model.type] boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:13]} context:nil].size;
            ...
            CGSize size2=CGSizeMake(self.view.frame.size.width-10-105-70, MAXFLOAT);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.tag==100)
    {
        ...
        JTShangpinListTabViewCell * customCell=[tableView dequeueReusableCellWithIdentifier:@"customCell"];
        if (!customCell)
        {
            customCell=[[JTShangpinListTabViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"customCell"];
        ...
        if ([[NSString stringWithFormat:@"%@",model.score] isEqualToString:@""]||model.score==nil||[model.score intValue]==0)
        {
            customCell.priceLab.frame=CGRectMake(self.view.frame.size.width-70-10, 60, 70, 40);
            customCell.priceLab.text=@"暂无评价";
            customCell.priceLab.textColor=[UIColor orangeColor];
            customCell.priceLab.font=[UIFont systemFontOfSize:15];
            UILabel * lab=(UILabel *)[customCell viewWithTag:1000];
            lab.hidden=YES;
        }
        else
        {
            customCell.priceLab.frame=CGRectMake(self.view.frame.size.width-30-25, 60, 30, 40);
            float scroe=[model.score floatValue];
            customCell.priceLab.text=[NSString stringWithFormat: @"%.1f",scroe] ;
            customCell.priceLab.textColor=[UIColor redColor];
            customCell.priceLab.font=[UIFont systemFontOfSize:18];
            UILabel * lab=(UILabel *)[customCell viewWithTag:1000];
            lab.text=@"分";
            lab.hidden=NO;
            lab.frame=CGRectMake(self.view.frame.size.width-10-20, 85, 10, 10);
            
        }

        customCell.shangjiaNameLab.text=model.fuName;
        ...
        if ([[NSString stringWithFormat:@"%@",model.type] isEqualToString:@""]||model.type==nil)
        {
            ...
            customCell.typeLab.frame=CGRectMake(150, 65, self.view.frame.size.width-10-150-70, 20);
        }
        else
        {
        ...
        customCell.typeLab.frame=CGRectMake(150, 65,self.view.frame.size.width-10-150-70, autoSize.height);
        if (model.street==nil||[model.street isEqualToString:@""])
        {
            model.street=@"";
        }
        ...
        customCell.quLab.frame=CGRectMake(105, 65+customCell.typeLab.frame.size.height, self.view.frame.size.width-10-105-70,autoSize2.height);
        customCell.bgImgView.frame=CGRectMake(0, 5, self.view.frame.size.width,80+customCell.typeLab.frame.size.height+customCell.quLab.frame.size.height);
    ...
    switch (tableView.tag-100)
    {
        ...
        case 4:
        {
            ...
            if (indexPath.row==0)
            {
                ...
                NSMutableArray * typeArr=[[NSMutableArray alloc] initWithArray:[_typeTitleArr objectAtIndex:clickTypeNum]];
...
-(void)sendPostGetAgePriceAndSoOn
{
    if ([SOAPRequest checkNet])
    {
        ...
        NSDictionary * jsondic=[[NSDictionary alloc] initWithObjectsAndKeys:@"商家联盟",@"key", nil];
...
-(void)GetCompanyListData
{
    if ([SOAPRequest checkNet])
    {
        ...
        NSDictionary * zaojiaoListDic=[[NSDictionary alloc] initWithDictionary:[SOAPRequest getResponseByPostURL:[NSString stringWithFormat:@"%s%s",QYL_URL,NEW_QYL_ShangPin_shopGoodsPage] jsonDic:jsondic]];
        
        if ([[NSString stringWithFormat:@"%@",[zaojiaoListDic objectForKey:@"resultCode"]] isEqualToString:@"1000"])
        {
            if (q==3)
            {
                if ([[[zaojiaoListDic objectForKey:@"goodsPage"] objectForKey:@"totalCount"] intValue]==0)
                {
                    NSString * str=@"未找到符合条件的数据！";
                    [JTAlertViewAnimation startAnimation:str view:self.view];
                    return;
                }
                NSArray * zaojiaoListArr=[[zaojiaoListDic objectForKey:@"goodsPage"] objectForKey:@"list"];
                for (int i=0; i<zaojiaoListArr.count; i++)
                {
                    ...
                    sortModel.fuName=[[zaojiaoListArr objectAtIndex:i] objectForKey:@"shopName"];
...
-(void)loadMoreData
{
    if ([SOAPRequest checkNet])
    {
        ...
        if ([[NSString stringWithFormat:@"%@",[zaojiaoListDic objectForKey:@"resultCode"]] isEqualToString:@"1000"])
        {
            if ([[[zaojiaoListDic objectForKey:@"goodsPage"] objectForKey:@"list"] count]==0)
            {
